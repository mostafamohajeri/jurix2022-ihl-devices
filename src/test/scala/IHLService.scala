...
import asl.{ihl_advisor, ihl_device, ihl_scenario}
import bb.expstyla.exp.{BooleanTerm, StringTerm, StructTerm, VarTerm}
...
val logger = PlantUMLCommunicationLogger()
  ...
  Environment.logActors(Seq("IHLScenario", "IHLDevice","IHLAdvisor"))
  ...
  override def beforeAll(): Unit = {
    ...
    val result: Future[IMessage] = system.ask(ref => AgentRequestMessage(
      Seq(
        ...
        AgentRequest(new ihl_scenario(coms = loggableComs).agentBuilder, "IHLScenario", 1),
  ...
  "ihl advisor" should {
    "init" in {
      ...
      mas.yellowPages.getAgent("IHLScenario").get.asInstanceOf[AkkaMessageSource].address()  ! GoalMessage(
        StructTerm("run",Seq()),AkkaMessageSource(prob.ref)
      )
//      val message = prob.receiveMessage();
    ...
    "return true if it has a fact" in {
      val d : FiniteDuration = Duration.apply(10,scala.concurrent.duration.SECONDS)
      val prob = testKit.createTestProbe[IMessage]()
      ...
      val message = prob.receiveMessage(d)
